...
class KwargsFormatter:
    ...
    def _repr_str(self, value: Any) -> str | None:
    ...
    def _repr_collection(self, value: Any) -> str | None:
        if isinstance(value, (list, tuple, set)) and len(value) * 3 > self._max_value_length:
            shape = (len(value),)
            return self._serialize_as_class(value, shape)

        if isinstance(value, dict) and len(value) * 6 > self._max_value_length:
            shape = (len(value),)
            return self._serialize_as_class(value, shape)
    ...
    def _format_ndim_array(cls, value: Any) -> str | None:
        if hasattr(value, "shape") and isinstance(value.shape, tuple):
            return cls._serialize_as_class(value, value.shape)

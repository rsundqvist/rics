...
class MultiCaseTimer(Generic[DataType, *Ts]):
    ...
    Memory:
        When `test_data` is a callable, variants are generated lazily and **at most one is held in memory at a time**
        -- during calibration (including the ``stratify="auto"`` probe) as well as timing. This lets the timer
        benchmark high memory-pressure workloads whose variants cannot all coexist; the cost is that a variant may be
        regenerated across calibration rounds. Pass a callable (rather than a pre-built ``{label: data}`` dict, which
        necessarily holds every variant) to get this guarantee.
    ...
    Args:
        ...
        test_data: A ``{label: data}`` to evaluate candidates on. You may also pass a list of data, which will be
            converted to a dict as above. Data may also be generated by passing a callable -- generated lazily, one
            variant in memory at a time (see *Memory* above), suitable for high memory-pressure workloads.

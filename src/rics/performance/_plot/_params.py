...
# Keyword aliases accepted for the `x`/`hue` arguments, in addition to test-data dimension names. Lookups are
# lowercased, so the canonical column names (FUNC/DATA) are matched via their lowercase forms below.
_ALIASES: dict[str, str] = {"candidate": FUNC, "func": FUNC, "data": DATA, "test data": DATA}

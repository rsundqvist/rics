    ...
    **plot_kwargs: Any,
    ...
    Args:
        ...
        test_data: A ``{label: data}`` to evaluate candidates on. You may also pass a list of data, which will be
            converted to a dict as above. Data may also be generated by passing a callable -- generated lazily, with
            at most one variant in memory at a time (see :class:`.MultiCaseTimer`), suitable for high memory-pressure
            workloads.
